...
void generateMoves(int& depth, board& state)
{
    ...
    for(unsigned long long m = cloneMask; m >= 1; m -= (m&(-m)))
        {ax.init(0, 0, (63 - __builtin_clzll((m&(-m)))), state.getMovingPlayer()); moves[depth][movesSize[depth]++] = ax;}
    for(m = playerMask; m >= 1; m -= (m&(-m)))
    {
        ...
        for(playerMask = cloneMask; playerMask >= 1; playerMask -= (playerMask&(-playerMask)))
            {ax.init(1, (63 - __builtin_clzll(m&(-m))), (63 - __builtin_clzll(playerMask&(-playerMask))), state.getMovingPlayer()); moves[depth][movesSize[depth]++] = ax;}
    }
}